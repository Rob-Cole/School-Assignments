...
public class Geometry {
	...
	public static boolean isInside(Point2D.Double p, Ellipse2D.Double e) {
		...
		if (Math.pow(p.getX()-e.getCenterX(),2)/Math.pow(e.getWidth()/2,2)+Math.pow(p.getY()-e.getCenterY(),2)/Math.pow(e.getHeight()/2,2) <=1) {
			return true;
		}
	...
	public static boolean isOnBoundary(Point2D.Double p, Ellipse2D.Double e){
		...
		if (Math.floor(Math.pow(p.getX()-e.getCenterX(),2)/Math.pow(e.getWidth()/2,2)+Math.pow(p.getY()-e.getCenterY(),2)/Math.pow(e.getHeight()/2,2)) == 1){
			return true;
		}
	return false;
	}
...
public static void main (String [] args){
	...
	if (isInside(p,e)== true) {
		System.out.print("true");
	}
	else 
		System.out.print("false");
	
	if (isOnBoundary(p,e) == true) {
		System.out.print("true");
	}
